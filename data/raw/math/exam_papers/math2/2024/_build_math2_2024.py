...
YEAR = 2024
RAW_FILE = ROOT / "_raw_questions_2024.json"
RAW_PY_FILE = ROOT / "_raw_math2_2024.py"
...
def score_for(number: int) -> int:
    if number <= 16:
        return 5
    return 10 if number <= 20 else 11
...
def load_questions() -> list[Question]:
    ...
    if RAW_PY_FILE.exists():
        raw_items = runpy.run_path(str(RAW_PY_FILE))["QUESTIONS"]
        for item in raw_items:
            number = item["number"]
            source_pages = item.get("source_pages") or [question_page(number)]
            answer_pages = item.get("answer_pages") or [answer_page(number)]
            assets = [f"images/source_pages/page-{page}.png" for page in source_pages]
            assets.extend(f"images/answer_pages/page-{page}.png" for page in answer_pages)
            questions.append(
                Question(
                    number=number,
                    question_type=item["question_type"],
                    score=item.get("score", score_for(number)),
                    module=item["module"],
                    topics=item["topics"],
    ...
    for item in raw_items:
        ...
        questions.append(
            Question(
                number=number,
                question_type=qtype_map[item["question_type"]],
